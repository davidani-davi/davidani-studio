import {contourFaceMask} from '../lib/contour-face-mask';
...
async function run(view:'full',anchorImageUrl?:string){
 const publicPath='/models/studio 102/full.png',source=fs.readFileSync(path.join(out,'reference.png'));
 const ref=await referencePixels(source),referenceUrl=await upload(source,`celine-3-${view}.png`);
 const shot=simpleReferenceShot({view,referenceUrl,garmentImageUrls:[garment],anchorImageUrl,category:'top',framing:'full'});
 ...
 const contour={width:ref.width,height:ref.height,sha256:ref.sha256,points:[[405,93],[468,103],[500,142],[512,196],[487,229],[457,253],[437,257],[409,241],[389,209],[382,164],[391,118]] as [number,number][],featherPixels:16};
 const mask=contourFaceMask(ref,contour);fs.writeFileSync(path.join(out,'contour.json'),JSON.stringify(contour,null,2));
