...
(async () => {
  ...
  const ctx = await browser.newContext({viewport:{width:1440,height:1080}, reducedMotion:'reduce'});
  ...
  for(const view of ['Front','Side','Back']) assert(await page.getByRole('button',{name:view,exact:true}).count());
