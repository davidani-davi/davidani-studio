...
(async () => {
  ...
  assert.equal(await options.count(),1+references.length);
  ...
  await page.getByRole('button',{name:'DP62087 · Camo · 3/4 length',exact:true}).click();
  await page.getByRole('button',{name:/Generate 3 views/}).first().waitFor();
  assert.deepEqual(errors,[]);
  await browser.close();
  console.log('PASS: DP style selects all shared pants options, including the 3/4-length DP62087; three views, no full, all reference thumbnails loaded. No paid generation.');
