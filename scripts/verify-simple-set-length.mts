/** Live regression: actual DTP68208 product + Celine 1, retain raw/final and pixel evidence. */
...
if(!productPath||!out)throw Error('Usage: vite-node scripts/verify-simple-set-length.mts PRODUCT.png OUTPUT_DIR');
...
const garment=await upload(fs.readFileSync(productPath),'DTP68208-blue-product.png');
async function run(view:'front'|'side'|'back'|'full',anchorImageUrl?:string){
 const publicPath=`/models/studio 98/${view}.png`,source=fs.readFileSync(path.join('public',publicPath));
 const ref=await referencePixels(source),referenceUrl=await upload(source,`celine-1-${view}.png`);
 const shot=simpleReferenceShot({view,referenceUrl,garmentImageUrls:[garment],anchorImageUrl,category:'set',framing:'full',garmentName:'Braided Trim Ruffle Chambray Wide Leg Set'});
 ...
 const mask=simpleFaceMask(ref,publicPath,view,'full')!;
...
await Promise.all(['side','back','full'].map(view=>run(view as 'side'|'back'|'full',anchor)));
