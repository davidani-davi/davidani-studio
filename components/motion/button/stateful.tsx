"use client";
// beui.dev/components/motion/button — vendored, unmodified.

import { Check, Loader2, X } from "lucide-react";
import { AnimatePresence, motion, useReducedMotion, type Variants } from "motion/react";
import { forwardRef, type ReactNode, useEffect, useLayoutEffect, useRef, useState } from "react";

/**
 * DEVIATION FROM UPSTREAM beUI.
 *
 * TextSlot measures its own label in a layout effect. Next server-renders
 * client components for the initial HTML, so upstream's bare useLayoutEffect
 * warns on every server render of this button — and this is the studio's
 * Generate button, so that is every page load. Same effect on the client,
 * silent on the server, which is what the measurement wants anyway: there is
 * nothing to measure without a layout.
 *
 * Keep this shim if beUI's button is ever re-pulled.
 */
