...
export function houseModelContinuity(modelId: string, view: PresetView, anchor: string, origin: string) {
  ...
  return {
    ...
    rule: `House model continuity: the canvas is the current clean front identity reference. ${direction} Change the viewpoint and framing as requested; do not preserve its front-facing pose or crop. Preserve her exact facial identity, hair color, age, natural skin finish and body proportions. Keep skin clean with restrained fine texture; do not add gritty pores, artificial grain, waxy smoothing or sharpening. The garment references remain the source of truth for the garment.`,
  };
}
