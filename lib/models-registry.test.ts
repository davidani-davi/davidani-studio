  ...
  it("offers three looks each for Vision and Celine, plus a dedicated DONUTS pants reference", () => {
    ...
    for (const model of visible.filter(m => m.id !== "studio 103")) {
      ...
      for (const framing of ["crop", "low"] as const) {
        ...
        // a look maps to its derived plate when one is installed, else stays on itself
        // (the newer outfit sets have no low plate; the neutral-black sets have no crop plate either)
        const derived = model.id.replace("studio", framing);
        expect(mapped.humanModelId).toBe(models.some(m => m.id === derived) ? derived : model.id);
      }
    }
  });
});
