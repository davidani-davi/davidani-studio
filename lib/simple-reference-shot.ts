...
export function simpleReferenceShot(o: ReferenceShot) {
  ...
  const prompt = `Edit image 1 as the exact base photograph. Replace only the ${scope} with the garment in image 2. Match its color, fabric, cut, length, print and construction. Keep image 1's exact pose, framing, background, lighting and other clothing. Preserve the face, expression, head position and hairstyle unchanged. No sharpening, retouching or enhancement. One clean photograph of one person.${inferred ? ' No back garment photo is supplied; use a plain continuation of the fabric without invented rear graphics.' : ''}${o.color ? ` Garment color: ${o.color}.` : ''}${o.note ? ` Requested change: ${o.note}` : ''}`;
...
export function simpleFaceMask(ref: ReferencePixels, publicPath: string, view: string, framing: string): Buffer | undefined {
  if (framing === 'low' || view === 'back') return undefined;
  const preset = presets[publicPath.split('?')[0] as keyof typeof presets];
