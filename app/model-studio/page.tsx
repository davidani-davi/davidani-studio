...
export default function ModelStudioPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await fetchJson("Load models", "/api/models");
        const models: HumanModel[] = data.models ?? [];
        setHumanModels(models);
        if (models.length > 0) {
          setSelectedHumanModelId(models[0].id);
          if (models[0].poses[0]) setSelectedPoseId(models[0].poses[0].id);
        }
      } catch (err: any) {
    ...
    <main className="flex h-screen flex-col bg-neutral-50">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <PromptPanel
          ...
          onTwoPieceChange={(v) => {
            setTwoPiece(v);
            // Invalidate the prompt — it was written for the other mode. The
            // user needs to re-run Analyze before Generate is trustworthy.
            setPrompt("");
          }}
        />
